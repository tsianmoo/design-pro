...
export const useSettingStore = defineStore({
  ...
  actions: {
    ...
    initState() {
      ...
      if (sys) {
        ...
        if (setting) {
          ...
          this.menuOpenWidth = setting.menuOpenWidth || defaultMenuWidth
          ...
          this.boxBorderMode = setting.boxBorderMode ?? true
          this.uniqueOpened = setting.uniqueOpened ?? true
          this.showMenuButton = setting.showMenuButton ?? true
          this.showRefreshButton = setting.showRefreshButton ?? true
          this.showCrumbs = setting.showCrumbs ?? true
          this.autoClose = setting.autoClose ?? false
          this.showWorkTab = setting.showWorkTab ?? true
          this.showLanguage = setting.showLanguage ?? true
          this.showNprogress = setting.showNprogress ?? true
          this.colorWeak = setting.colorWeak ?? false
          this.showSettingGuide = setting.showSettingGuide ?? true
          this.pageTransition = setting.pageTransition || 'slide-right'
          this.menuOpen = setting.menuOpen ?? true
          this.watermarkVisible = setting.watermarkVisible ?? false
          this.customRadius = setting.customRadius || defaultCustomRadius
          this.holidayFireworksLoaded = setting.holidayFireworksLoaded ?? false
          this.showFestivalText = setting.showFestivalText ?? false
          this.festivalDate = setting.festivalDate || ''
          this.dualMenuShowText = setting.dualMenuShowText ?? false
          this.containerWidth = setting.containerWidth || ContainerWidthEnum.FULL
        }
      }

      // 设置主题色
      handleElementThemeColor(this.systemThemeColor, this.isDark)
    ...
    setCustomRadius(radius: string) {
      ...
      // 确保在设置圆角时立即应用CSS变量，使用rem单位
      document.documentElement.style.setProperty('--custom-radius', `${radius}rem`)
      console.log('设置自定义圆角:', radius, '应用CSS变量:', `${radius}rem`)
